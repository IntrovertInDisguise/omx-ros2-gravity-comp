...
private:
  ...
  // Backwards-compatible names used in cpp
  KDL::Vector K_trans_homing_;
  KDL::Vector D_trans_default_;
  KDL::Vector D_rot_;
  ...
  // Convenience / alternative interface lists (some code variants use these)
  std::vector<std::reference_wrapper<const hardware_interface::LoanedStateInterface>> joint_state_interfaces_;
  std::vector<std::reference_wrapper<const hardware_interface::LoanedStateInterface>> joint_velocity_interfaces_;
  std::vector<std::reference_wrapper<hardware_interface::LoanedCommandInterface>> tau_cmd_interfaces_;

  // Joint value buffers
  std::vector<double> joint_positions_;
  std::vector<double> joint_velocities_;
  std::vector<double> prev_cmd_torque_;

  // Cartesian frames and history
  KDL::Frame x_d_frame_;
  KDL::Frame x_c_start_;
  KDL::Frame x_c_frame_;
  KDL::Twist last_x_error_;
  KDL::Twist x_dot_error_filtered_;
  bool first_update_ = true;
  double move_start_time_ = 0.0;

  // Homing helpers
  KDL::Frame homing_start_frame_;   // EE pose when homing starts
  double homing_start_time_ = 0.0;

  // Waypoint safety/clamping defaults (user accepted)
  double max_waypoint_distance_ = 0.2; // meters
  double max_waypoint_rate_ = 0.1;     // meters / second

  // Homing PD gains (gentle by default)
  double homing_kp_ = 10.0;
  double homing_kd_ = 0.5;
  ...
  bool waypoint_mode_active_ = false;
  ...
  // Safety limits (fallback constants)
  static constexpr double MAX_CARTESIAN_STIFFNESS = 10000.0;
  static constexpr double MAX_CARTESIAN_DAMPING = 1000.0;
  static constexpr double MAX_ROTATIONAL_STIFFNESS = 10000.0;
  static constexpr double MAX_ROTATIONAL_DAMPING = 1000.0;
