...
// (old on_activate implementation removed)
...
controller_interface::CallbackReturn OmxVariableStiffnessController::on_activate(const rclcpp_lifecycle::State &)
{
  // 1. Clear buffers
  ...
  for (size_t i = 0; i < joints_.size(); ++i) {
    if (i >= pos_if_.size() || !pos_if_[i]) {
      RCLCPP_ERROR(get_node()->get_logger(), "Joint position interface missing for '%s'", joints_[i].c_str());
      return CallbackReturn::ERROR;
    }
    double pos = pos_if_[i]->get_value();
    if (std::isnan(pos)) {
  ...
  x_d_frame_ = x_c_frame_;  // <--- The Magic Line. Target = Current.
  ...
  RCLCPP_INFO(get_node()->get_logger(), "Activated. Holding current pose: [%f, %f, %f]", 
    x_c_frame_.p.x(), x_c_frame_.p.y(), x_c_frame_.p.z());
...
OmxVariableStiffnessController::update(const rclcpp::Time & time, const rclcpp::Duration & period)
{
  ...
  // }
  for (size_t i = 0; i < joints_.size(); ++i) {
    // Send computed torque to hardware command interface (if available).
    // Note: `tau_total` already includes torque_scale_ above, so do not scale again here.
    if (i < eff_cmd_if_.size() && eff_cmd_if_[i]) {
...
void OmxVariableStiffnessController::publish_manipulability_metrics_()
{
  ...
  Eigen::MatrixXd J = Eigen::MatrixXd(jacobian_.data);                    // 6xN Jacobian
