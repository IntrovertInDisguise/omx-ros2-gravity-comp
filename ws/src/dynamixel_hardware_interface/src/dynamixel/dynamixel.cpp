  ...
  bool is_fast)
{
  ...
  // Note: Do NOT override packet timeout with period_ms.
  // The SDK calculates appropriate timeout from expected data size and baud rate.
  // Overriding with the control loop period (e.g. 5ms at 200Hz) is too aggressive
  // for multi-servo sync/bulk reads, especially over USB with latency.
  (void)period_ms;
