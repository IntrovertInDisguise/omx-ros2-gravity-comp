...
void DynamixelHardware::CalcTransmissionToJoint()
{
  ...
  // Map Present Current -> effort (for current/torque feedback)
  this->MapInterfaces(
    num_of_joints_,
    num_of_transmissions_,
    hdl_joint_states_,
    ...
    "",  // No conversion needed for effort
    std::function<double(double)>()
  );
}
...
void DynamixelHardware::CalcJointToTransmission()
{
  ...
  // Map Goal Position -> position (for position control mode)
  this->MapInterfaces(
    num_of_transmissions_,
    num_of_joints_,
    hdl_trans_commands_,
  ...
  // Map Goal Current -> effort (for current/torque control mode)
  this->MapInterfaces(
    num_of_transmissions_,
    num_of_joints_,
    hdl_trans_commands_,
    ...
    dynamixel_hardware_interface::dxl_to_ros2_cmd_map,
    "Goal Current",
    "",  // No conversion needed for current
    std::function<double(double)>()
  );
}
